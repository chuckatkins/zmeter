/* rs22812.c - Radio Shack 22-812 digital multimeter with RS-232 interface
...
#if 0  /* :TODO: provide support for letters '0' and 'S' */
...
static void cook_value( RS22812_PACKET* packet )
{
  ...
  if (packet->neg )                       digit_str[i++] = '-';
  digit_str[i++] = packet->digit_1.digit;
  if (packet->digit_2.seg_pt)             digit_str[i++] = '.';
  digit_str[i++] = packet->digit_2.digit;
  if (packet->digit_3.seg_pt)             digit_str[i++] = '.';
  digit_str[i++] = packet->digit_3.digit;
  if (packet->digit_4.seg_pt)             digit_str[i++] = '.';
...
/* "constructor" */
RS22812_PACKET* rs22812_packet_new( void )
{
...
/* "destructor" */
void rs22812_packet_delete( RS22812_PACKET* packet )
{
  assert(packet);
  free(packet);
}
...
void rs22812_packet_dump( RS22812_PACKET* packet)
{
  int i;
  assert(packet);
...
void rs22812_packet_read( RS22812_PACKET* packet, char* buf )
{
  assert( packet );
...
void rs22812_packet_cook( RS22812_PACKET* packet )
{
  assert(packet);
...
void rs22812_packet_sprint( RS22812_PACKET* packet, char* buf, int bufsize )
{
  assert( packet );
  assert( buf );
